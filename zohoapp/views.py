...
def edit_expense(request, expense_id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'edit_expense.html', {'expense': expense,'company':company, 'vendors': vendors, 'customers': customers,'accounts': accounts,'selected_account': selected_account,'items':  account_types,'payments':payments})
    ...
    if request.user.is_authenticated:
        if request.method=='POST':
            ...
            sales=SalesOrder(customer_id=custo,sales_no=sales_no,terms=term,reference=reference, sales_date=sa_date,ship_date=sh_date,
                        cxnote=cxnote,subtotal=subtotal,igst=igst,cgst=cgst,sgst=sgst,t_tax=totaltax,
                        grandtotal=t_total,status=status,terms_condition=tc,file=file,sos=sos,sh_charge=sh_charge,user=user)
            sales.save()
...
def create_purchase_bill1(request):
    ...
    if request.method == 'POST':
        ...
        bill = PurchaseBills(user=user,cus_name_id=custo, customer_name=cust_name,customer_email= cust_email,place_of_supply=pos,vendor_name=vendor_name,
...
# RECURRING INVOICE NEW CORRECTION WORK..........................................................
...
def create_recur(request):
    ...
    return render(request,'samrecurpage.html',{'item':item,'cus':cus,'pay':pay,'company':company,'every':every,'banks':banks,})
...
def edit_recur(request,id):
    ...
    return render(request,'edit_recur.html',{'editr':editr,'banks':banks,'cus':cus,'tab':tab,'item':item,'pay':pay,'every':every,'company':company})
